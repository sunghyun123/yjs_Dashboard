...
- 로그인은 **카카오 OAuth** 방식을 사용합니다.
- 허용 사용자 목록은 프로젝트 루트의 `kakao_whitelist.json`에서 관리합니다.
  - 최초 관리자는 `kakao_whitelist.json`에 본인 카카오 ID를 `role: "admin"`으로 등록한 뒤 카카오 로그인을 진행합니다.
- 화이트리스트에 없는 사용자가 로그인하면 관리자 승인 대기 상태가 되며, 관리자 화면(`/admin.html`)에서 승인할 수 있습니다.
...
- 로그인 버튼 클릭 시 카카오 로그인 페이지로 이동하고, 인증 후 상황판으로 돌아옵니다.
- AI 비서가 필요하면 상단 버튼으로 `index.html`로 이동합니다.
