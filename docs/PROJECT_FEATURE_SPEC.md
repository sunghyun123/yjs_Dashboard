...
- 프론트엔드: 정적 HTML (`index.html`, `dashboard.html`, `admin.html`) + 레거시 경로(`board.html`)
- AI:
...
- 로그인 보조 액션:
  ...
  - `계정 찾기` 버튼: 이름 입력 후 저장된 ID 조회 + 비밀번호 재설정
- 초기 관리자 기본 계정:
  - `admin / 1234`
- 채팅 입력 카테고리:
  - `schedule_create`
...
- 일정 목록 조회 및 렌더링
- 루트 경로(`/`) 기본 진입 화면으로 동작
- 비로그인 접속 시 페이지 내 로그인 모달 표시, 로그인 성공 후 초기화
- 기본 조회 정책:
  - `오늘만` ON: 당일 조회
...
- 외출/행선표 상태 조회
- 외출/행선표 전용 인원 목록 기반 상태 편집
  - 상태 클릭: `사무실/외출/야간작업` 순환
  - 행선/복귀 시각은 `외출` 상태에서만 노출
  - 복귀 시각은 모달 시간 선택 + `▲/▼` 30분 단위 증감
- 일반 메모 등록/조회/삭제
- 전자칠판 템플릿 입력 지원
- 상세(`details`) 표시 정책
  - 2줄 이하: 카드에 직접 표시
  - 3줄 이상: `상세 보기` 버튼으로 모달 표시
...
## 4.4 전자칠판 레거시 경로 (`/board.html`)

- `board.html` 정적 파일은 제거되었고, 경로 호환만 유지한다.
- 런타임에서 `/board.html` 접근 시 `307`으로 `/dashboard.html`로 리다이렉트한다.
- 신규 기능은 `dashboard.html` 기준으로만 반영한다.
- 런타임 루트(`/`) 접근 시 `dashboard.html`을 기본 페이지로 제공한다.
...
- 비밀번호는 해시(`password_hash`)만 저장하며, API 응답에 비밀번호를 포함하지 않는다.
- 응답:
  - 성공 시 `success`
  - 중복 ID 등 실패 시 `400`
...
### `GET /api/schedules/outing-staff`
...
### `POST /api/admin/outing-staff`

- 외출/행선표 전용 인원 추가

### `DELETE /api/admin/outing-staff/{staff_id}`
...
- `users` 테이블은 `user_name`, `password_hash` 기반으로 인증하며 평문 비밀번호 저장을 금지한다.
...
- 계정 찾기는 비밀번호 표시가 아닌 재설정 방식(`POST /api/auth/reset-password`)으로 동작한다.
...
- 레거시 `board.html` 직접 렌더링 정책은 종료되었고, 현재 런타임은 `dashboard.html`로 리다이렉트된다.

### 13.7 같은 날짜 내부 정렬 우선순위

- 같은 날짜의 일정 노출 순서는 아래를 따른다.
  - `공사 일정(주간)`
  - `공사 일정(야간)`
  - `공사 일정(심야)`
  - `일정`
  - `일반 작업`
- `공사 일정` 내부 주/야/심야 구분은 일정 태그(`tags`) 값을 기준으로 판정한다.
...
- `board.html` 경로는 레거시 호환을 위해 URL만 유지하고, 실제 화면 동작은 `dashboard.html` 기준으로 통일한다.
