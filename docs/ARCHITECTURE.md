...
- 페이지: `/`, `/dashboard.html`, `/admin.html`, `/board.html`(접속 시 `dashboard.html`로 307 리다이렉트)
...
- `dashboard.html`
  ...
  - **전자칠판 템플릿 입력**: 등록/메모 구분 드롭다운을 제거하고 **등록만** 가능하게 한다. 메모는 `기타 메모`에서만 작성한다(§11.20.5). **현장 위치**는 `공사 일정`·`일반 작업`·`일정` 전 카테고리에서 생략 가능하다(검증·안내 문구 동기화).
  - **로그아웃**: 주요 화면 상단에 로그아웃 액션을 제공하고, 실행 시 세션 종료 후 로그인 화면으로 이동한다(§11.21.2).
